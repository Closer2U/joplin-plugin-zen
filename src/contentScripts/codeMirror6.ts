...
const mobileFillers = [
  'actually', 'basically', 'just', 'kind of', 'really', 'sort of', 'very', 'quite', 'rather',
];
...
const mobileCliches = [
  'at the end of the day', 'down the drain', 'on the same page', 'think outside the box', 'tip of the iceberg',
];
...
const mobileRedundancies = [
  'basic fundamentals', 'combine together', 'end result', 'past history', 'personal opinion', 'return back',
];

const desktopWeakPhrases = [
  'is able to', 'is due to', 'is going to', 'it seems', 'there is', 'there are', 'was able to', 'were able to',
];
const mobileWeakPhrases = [
  'there is', 'there are', 'is going to',
];
...
const irregularVerbLemmas: Record<string, string> = {
...
const parseCustomPatterns = (source: string) => {
  ...
  for (const rawLine of source.split(/\r?\n/g)) {
    ...
    if (body.startsWith('/') && body.lastIndexOf('/') > 0) {
      ...
      try {
        regex = new RegExp(pattern, flags.includes('g') ? flags : `g${flags}`);
      } catch (_error) {
        regex = null;
      }
    } else {
...
const desktopTokensFromWink = (text: string, offset = 0): Token[] => {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    tokens.push({
      ...
      from: offset + foundIndex,
      to: offset + foundIndex + value.length,
      prevLower: '',
      prev2Lower: '',
      nextLower: '',
      next2Lower: '',
      lemma: String(lemmas[i] || value).toLowerCase(),
      posTag: String(poses[i] || ''),
    });
...
const looksLikeLikelyNoun = (lower: string) => {
  return desktopNounLexicon.has(lower)
    || nounSuffixes.some(s => lower.endsWith(s))
    || nounEnds.has(lower)
    || (lower.endsWith('s') && lower.length > 3 && !lower.endsWith('ly'));
};

const determinerBasedNoun = (token: Token) => {
  return determiners.has(token.prevLower) || determiners.has(token.prev2Lower);
};
...
const classifyWordMobile = (token: Token, settings: ZenSettings): SyntaxKind | null => {
  ...
  if (settings.syntaxHighlightVerbs && (
    mobileVerbLexicon.has(lower)
    || mobileVerbLexicon.has(lemma)
    || ((pronouns.has(token.prevLower) || mobileConjunctions.has(token.prevLower) || intensifiers.has(token.prevLower) || pronouns.has(token.prev2Lower)) && /(s|es|ies)$/.test(lower))
    || (gerundLike && !determinerBasedNoun(token) && (gerundPrepositions.has(token.prevLower) || determiners.has(token.nextLower) || looksLikeLikelyNoun(token.nextLower) || looksLikeLikelyNoun(token.next2Lower) || token.nextLower === 'and'))
    || /(ed|en|fy|ise|ize)$/.test(lower)
  )) return 'verb';

  if (settings.syntaxHighlightAdjectives && (
    mobileAdjectiveLexicon.has(lower)
    || (intensifiers.has(token.prevLower) && !mobileConjunctions.has(token.nextLower))
    || adjectiveSuffixes.some(s => lower.endsWith(s))
  )) return 'adjective';

  if (settings.syntaxHighlightNouns && (
    mobileNounLexicon.has(lower)
    || mobileNounLexicon.has(lemma)
    || determinerBasedNoun(token)
    || nounSuffixes.some(s => lower.endsWith(s))
    || nounEnds.has(lower)
  )) return 'noun';
...
const classifyWordDesktopHeuristic = (token: Token, settings: ZenSettings): SyntaxKind | null => {
  ...
  if (scores.adverb >= 6) {
    return settings.syntaxHighlightAdverbs ? 'adverb' : null;
  }
...
const classifyToken = (token: Token, settings: ZenSettings) => {
  return settings.platform === 'desktop' ? classifyWordDesktop(token, settings) : classifyWordMobile(token, settings);
};
...
const collectRegexMatches = (
  ...
  offset: number,
  regexes: RegExp[],
  kind: Exclude<StyleKind, 'custom' | 'repetition'> | 'custom',
  exceptions: RegExp[],
) => {
  const matches: MarkMatch[] = [];
  for (const originalRegex of regexes) {
    ...
    while ((match = regex.exec(text))) {
      ...
      if (matchedText.trim() && !exceptions.some(exception => new RegExp(exception.source, exception.flags).test(matchedText))) {
        matches.push({ from: offset + match.index, to: offset + match.index + match[0].length, kind });
      }
...
const collectRepetitionMatches = (text: string, offset: number, settings: ZenSettings) => {
  ...
  for (const token of tokens) {
    ...
    matches.push({ from: token.from, to: token.to, kind: 'repetition' });
  }
...
const buildSyntaxDecorations = (view: EditorView, settings: ZenSettings) => {
  ...
  if (settings.platform === 'desktop') {
    matches.push(...buildSyntaxMatches(fullText(view), 0, settings));
  } else {
    for (const range of view.visibleRanges) {
      matches.push(...buildSyntaxMatches(view.state.doc.sliceString(range.from, range.to), range.from, settings));
    }
  }
...
const buildStyleDecorations = (view: EditorView, settings: ZenSettings) => {
  ...
  const rules = buildStyleRules(settings);
  const custom = parseCustomPatterns(settings.styleCheckCustomPatterns);
  const matches: MarkMatch[] = [];

  const collectForBlock = (text: string, offset: number) => {
    for (const rule of rules) {
      matches.push(...collectRegexMatches(text, offset, [rule.regex], rule.kind, custom.exceptions));
    }
  ...
  if (settings.platform === 'desktop') {
    for (const paragraph of splitParagraphs(fullText(view), 0)) collectForBlock(paragraph.text, paragraph.offset);
  } else {
    for (const range of view.visibleRanges) {
      for (const paragraph of splitParagraphs(view.state.doc.sliceString(range.from, range.to), range.from)) {
        collectForBlock(paragraph.text, paragraph.offset);
      }
    }
  }

  return filterOverlaps(matches).map(match => Decoration.mark({ class: `jzm-stylecheck jzm-stylecheck-${match.kind}` }).range(match.from, match.to));
};
...
class ZenEditorState {
  ...
  private rebuildAll(forceCenter = false) {
    const decorationParts = [
      ...buildFocusDecorations(this.view, this.settings),
      ...this.cachedSyntaxDecorations,
      ...this.cachedStyleDecorations,
    ];
  ...
  public update(update: ViewUpdate) {
    if (update.docChanged || (update.viewportChanged && this.settings.platform === 'mobile')) this.scheduleAnalysis();
...
export default (context: ContentScriptContext) => {
  ...
  const zenExtension = (settings: ZenSettings) => [
    ...
    ViewPlugin.fromClass(class {
      ...
      public constructor(view: EditorView) {
        this.state = new ZenEditorState(view, settings);
        this.decorations = this.state.decorations;
      }
  ...
  const applySettings = (codeMirrorWrapper: any, input: unknown) => {
    ...
    codeMirrorWrapper.editor.dispatch({
      effects: zenCompartment.reconfigure(settings.enabled ? zenExtension(settings) : []),
    });
  };
  ...
  return {
    plugin: async (codeMirrorWrapper: any) => {
      ...
      try {
        applySettings(codeMirrorWrapper, await context.postMessage('getSettings'));
      } catch (_error) {
        applySettings(codeMirrorWrapper, defaultSettings);
      }
    },
